...
class AuthService {
  ...
  // ✅ Email sign up - Sets approval status based on role
  Future<User?> signUpWithEmail({
    ...
    try {
      ...
      final approvalStatus = role == 'doctor' ? 'pending' : 'approved';
      final profileComplete = role == 'patient' ? true : false;
      ...
      await _firestore.collection("users").doc(userCred.user!.uid).set({
        ...
        "approvalStatus": approvalStatus, // pending for doctors, approved for patients
      });
  ...
  Future<void> saveUserProfile({
    ...
    // Check if all steps are complete for doctors
    ...
    if (userDoc.exists) {
      ...
      if (role == 'doctor') {
        ...
        // All steps complete - ready for admin approval
        if (personalComplete && professionalComplete && availabilityComplete) {
          await _firestore.collection("users").doc(uid).update({
            'profileComplete': false, // Stays false until admin approves
  ...
  // ✅ Save doctor availability and mark as complete
  Future<void> saveDoctorAvailability({
    ...
    try {
      ...
      // Check if profile is now complete and ready for admin approval
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        if (role == 'doctor') {
          ...
          // All registration steps complete - now pending admin approval
          if (personalComplete && professionalComplete && availabilityComplete) {
            await _firestore.collection("users").doc(uid).set({
              'profileComplete': false, // Stays false until admin approves
              'approvalStatus': 'pending', // Ready for admin review
            }, SetOptions(merge: true));
            print("✅ Doctor profile submitted for admin approval");
          }
        }
      }
    } catch (e) {
